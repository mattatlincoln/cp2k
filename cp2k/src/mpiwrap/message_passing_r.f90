...
! *****************************************************************************
  SUBROUTINE mp_sum_rm(msg,gid)
    ...
    ENDDO
    t_end = m_walltime ( )
    CALL add_perf(perf_id=3,count=1,time=t_end-t_start,msg_size=msglensum*real_4_size)
#else
    MARK_USED(msg)
    MARK_USED(gid)
#endif
...
! *****************************************************************************
  SUBROUTINE mp_iscatter_r(msg_scatter,msg,root,gid,request)
    ...
    t_end = m_walltime ( )
    CALL add_perf(perf_id=4,count=1,time=t_end-t_start,msg_size=1*real_4_size)
#else
...
! *****************************************************************************
  SUBROUTINE mp_isendrecv_r(msgin,dest,msgout,source,comm,send_request,&
    ...
    t_end = m_walltime ( )
    CALL add_perf(perf_id=8,count=1,time=t_end-t_start,msg_size=2*real_4_size)
#else
    MARK_USED(dest)
    MARK_USED(source)
    MARK_USED(comm)
...
! *****************************************************************************
  SUBROUTINE mp_win_create_rv(base,comm,win)
    ...
    IF (len>0) THEN
       ...
       CALL mpi_win_create(base(lower1),len,real_4_size,MPI_INFO_NULL,comm,win,ierr)
    ELSE
...
! *****************************************************************************
  SUBROUTINE mp_rget_rv(base,source,win,win_data,myproc,disp,request,&
    ...
    REAL(kind=real_4), DIMENSION(:), POINTER CP_COMMA_CONTIGUOUS  :: base
    INTEGER, INTENT(IN)                                 :: source, win
    REAL(kind=real_4), DIMENSION(:), POINTER CP_COMMA_CONTIGUOUS  :: win_data
    ...
    t_end = m_walltime ( )
    CALL add_perf(perf_id=17,count=1,time=t_end-t_start,msg_size=SIZE(base)*real_4_size)
#else
    MARK_USED(source)
    MARK_USED(win)
    MARK_USED(myproc)
