...
! *****************************************************************************
  SUBROUTINE mp_sum_dm(msg,gid)
    ...
    ENDDO
    t_end = m_walltime ( )
    CALL add_perf(perf_id=3,count=1,time=t_end-t_start,msg_size=msglensum*real_8_size)
#else
    MARK_USED(msg)
    MARK_USED(gid)
#endif
...
! *****************************************************************************
  SUBROUTINE mp_iscatter_d(msg_scatter,msg,root,gid,request)
    ...
    t_end = m_walltime ( )
    CALL add_perf(perf_id=4,count=1,time=t_end-t_start,msg_size=1*real_8_size)
#else
...
!>      mpi_scatter
! *****************************************************************************
  SUBROUTINE mp_iscatter_dv(msg_scatter,msg,root,gid,request)
    REAL(kind=real_8), INTENT(IN)                      :: msg_scatter(:)
    REAL(kind=real_8), INTENT(INOUT)                   :: msg(:)
    INTEGER, INTENT(IN)                      :: root, gid
    INTEGER, INTENT(INOUT)                   :: request

    CHARACTER(len=*), PARAMETER :: routineN = 'mp_iscatter_dv', &
      routineP = moduleN//':'//routineN
...
! *****************************************************************************
  SUBROUTINE mp_rget_dv(base,source,win,win_data,myproc,disp,request,&
    ...
    REAL(kind=real_8), DIMENSION(:), POINTER CP_COMMA_CONTIGUOUS  :: base
    INTEGER, INTENT(IN)                                 :: source, win
    REAL(kind=real_8), DIMENSION(:), POINTER CP_COMMA_CONTIGUOUS  :: win_data
